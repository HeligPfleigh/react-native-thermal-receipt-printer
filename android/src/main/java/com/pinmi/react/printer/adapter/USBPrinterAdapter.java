...
public class USBPrinterAdapter implements PrinterAdapter {
  ...
  private static final byte[] SET_LINE_SPACE_24 = new byte[] {
    ESC_CHAR,
    0x33,
    24,
  };
  private static final byte[] SET_LINE_SPACE_32 = new byte[] {
    ESC_CHAR,
    0x33,
    32,
  };
      ...
      if (ACTION_USB_PERMISSION.equals(action)) {
        synchronized (this) {
          ...
          if (
            ...
            Log.i(
              LOG_TAG,
              "success to grant permission for device " +
  ...
  public void init(
    ...
    this.mUSBManager =
      (UsbManager) this.mContext.getSystemService(Context.USB_SERVICE);
  ...
  public void selectDevice(
    ...
    if (
      ...
      successCallback.invoke(
        new USBPrinterDevice(mUsbDevice).toRNWritableMap()
      );
      return;
    }
    closeConnectionIfExists();
    ...
    for (UsbDevice usbDevice : mUSBManager.getDeviceList().values()) {
      if (
        ...
        successCallback.invoke(
          new USBPrinterDevice(usbDevice).toRNWritableMap()
        );
        return;
      }
    }
    ...
    for (int i = 0; i < usbInterface.getEndpointCount(); i++) {
      ...
      if (ep.getType() == UsbConstants.USB_ENDPOINT_XFER_BULK) {
        if (ep.getDirection() == UsbConstants.USB_DIR_OUT) {
          UsbDeviceConnection usbDeviceConnection = mUSBManager.openDevice(
            mUsbDevice
          );
      ...
      new Thread(
        new Runnable() {
          ...
          public void run() {
            ...
            int b = mUsbDeviceConnection.bulkTransfer(
              mEndPoint,
              bytes,
              bytes.length,
              100000
  ...
  public static Bitmap getBitmapResized(Bitmap image, float decreaseSizeBy) {
    Bitmap resized = Bitmap.createScaledBitmap(
      image,
      (int) (image.getWidth() * decreaseSizeBy),
      (int) (image.getHeight() * decreaseSizeBy),
      true
    );
    return resized;
  }
